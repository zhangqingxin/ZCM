...
-(BOOL)checkLogin {
    ...
    if (appDelegate.username == nil || appDelegate.username == NULL) {
        [[[UIAlertView alloc] initWithTitle:@"提醒" message:@"请先登录！" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil] show];
        return NO;
    }
    return YES;
}
...
-(void)launcherViewItemSelected:(MyLauncherItem*)item {
    ...
    if ([[item controllerStr] isEqualToString:@"ContactsController"]) {
        if ([self checkLogin]) {
            [self initControllerWithName:[item controllerStr]];
        }
        return;
    } else if ([[item controllerStr] isEqualToString:@"AboutViewController"]) {
//        [self initControllerWithName:[item controllerStr]];
    } else {
        return;
    }
